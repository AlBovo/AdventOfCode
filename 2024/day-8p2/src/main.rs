    ...
    for i in positions {
        ...
        for j in 0..vec.len() {
            for e in j+1..vec.len() {
                ...
                let mut pos1_x = pos_j.0 + diff_x;
                let mut pos1_y = pos_j.1 + diff_y;

                let mut pos2_x = pos_e.0 - diff_x;
                let mut pos2_y = pos_e.1 - diff_y;


                while pos1_x >= 0 && pos1_x < matrix[0].len() as i32 && pos1_y >= 0 && pos1_y < matrix.len() as i32 {
                ...
                while pos2_x >= 0 && pos2_x < matrix[0].len() as i32 && pos2_y >= 0 && pos2_y < matrix.len() as i32 {
