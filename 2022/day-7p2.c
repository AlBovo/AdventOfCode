...
#define DEBUG
int rootSize, closest = 2e9;

int updateSize(Directory *dir){
    int total = 0;
    for(int i = 0; i < dir->dirs; i++){
        Directory *directory = (Directory *)dir->dir[i];
        total += updateSize(directory);
    }

    return (dir->size += total); // add inner dirs sizes
}

void dfs(Directory *dir){
    if((SIZE_MAX - rootSize) + dir->size >= SIZE){
        if(dir->size < closest)
            closest = dir->size;
    }
    for(int i = 0; i < dir->dirs; i++){
